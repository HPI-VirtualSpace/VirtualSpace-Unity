...
{
    public static Vector3 ClosestPointToPolygonLines(List<Vector3> polygon, Vector3 point)
    {
        ...
        {
            var lineVector = polygon[j] - polygon[i];
            var lineNormal = new Vector3(lineVector.x, -lineVector.z);
            var intersectionPoint = Intersection(polygon[i], polygon[j], point, point + lineNormal);

            var distance = Vector3.Distance(intersectionPoint, point);
            if (distance < minDist)
            {
                minDist = distance;
    ...
    public static Vector3 Intersection(Vector3 l1p1, Vector3 l1p2, Vector3 l2p1, Vector3 l2p2)
    {
        var denominator = (l2p2.z - l2p1.z) * (l1p2.x - l1p1.x) - (l2p2.x - l2p1.x) * (l1p2.z - l1p1.z);

        if (Math.Abs(denominator) < 10e-3) { return Vector3.zero; }

        var ua = ((l2p2.x - l2p1.x) * (l1p1.z - l2p1.z) - (l2p2.z - l2p1.z) * (l1p1.x - l2p1.x)) / denominator;
        //var ub = ((l2.X - l1.X) * (l1.Z - l3.Z) - (l2.Z - l1.Z) * (l1.X - l3.X)) / denominator;

        var x = l1p1.x + ua * (l1p2.x - l1p1.x);
        var z = l1p1.y + ua * (l1p2.z - l1p1.z);

        return new Vector3(x, 0, z);
    }

    // https://github.com/substack/point-in-polygon/blob/master/index.js
    public static bool PointInPolygon(List<Vector3> polygon, Vector3 point)
    {
        var x = point.x;
        var y = point.z;
